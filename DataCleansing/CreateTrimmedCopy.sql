CREATE PROCEDURE [DataCleansing].[CreateTrimmedCopy]
...
AS
	...
	IF (@TargetDatabaseName IS NULL) SELECT @TargetDatabaseName = @SourceDatabaseName + '_trimmed'

	-- *****************************************

	EXEC [DataCleansing].[CreateDatabaseCopy] @SourceDatabaseName, @TargetDatabaseName

	SET @Query = N'EXEC [' + @TargetDatabaseName + '].[DataCleansing].[ScrubBulkInformation]'
	EXEC (@Query)
